...
def construct_yolo_v3():
    ...
    f=open('yolo_name.txt', 'r', encoding="UTF-8")
...
def oneframe_size(  ):
    is_url = rtsp_url.lower().startswith(('rtsp://'))
    w = h = 0
...
while True:
    ...
    for i in range(len(res)):
        ...
        text=str(class_names[i])+'%.3f'%confidence

        frame = Image.fromarray(frame)
        draw = ImageDraw.Draw(frame)
        color= colors[id]
        draw.text((x1,y1-30),text,font=font,stroke_width=1 , fill=(int(color[0]),int(color[1]),int(color[2])))
