...
class YOLO:
    ...
    def __init__(self, model='yolov8n.pt', type='v8', cfg=DEFAULT_CFG) -> None:
        """
        ...
        if suffix == '.yaml':
            ...
            self._load(model, self.cfg2.data)
    ...
    def _load(self, weights: str, data: str):
        """
        ...
        if suffix == '.pt':
            ...
            #For Hangul processing, by Tory
            self.model.names = yaml_load(data)['names'] if data else {i: f'class{i}' for i in range(999)}
    ...
    def predict(self, source=None, stream=False, **kwargs):
        ...
        #User config by tory 
        overrides['conf'] = self.cfg2.conf
        overrides['show'] = self.cfg2.show
        overrides['imgsz'] = self.cfg2.imgsz
        overrides['iou'] = self.cfg2.iou
        overrides['data'] = self.cfg2.data
